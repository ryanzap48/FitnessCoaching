...
export default function Question6() {
  ...
  const [targetPounds, setTargetPounds] = useState(
    userAnswers.targetWeight ? String((userAnswers.targetWeight / 0.45359237).toFixed(0)) : ''
  );

  useEffect(() => {
    const poundsNum = parseInt(targetPounds, 10);
    if (!isNaN(poundsNum) && poundsNum > 99 && poundsNum < 501) {
      const totalkg = poundsNum * 0.45359237;
      updateAnswer('targetWeight', Math.round(totalkg * 100) / 100);
    }
}, [targetPounds, updateAnswer]);
  ...
  return (
     <QuestionLayout step={6} onContinue={handleNext}>
